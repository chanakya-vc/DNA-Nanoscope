...
%=== check inputs
% bioinfochecknargin(nargin, 2, mfilename); %%% DELETED FOR SPEED %%%%
%% DELETED FUNCTION CALL AND MOVED WORK INTO MAIN BODY
% [scoringMatrix, gapopen, gapextend, setGapExtend, scale, ...
% 	isAminoAcid, showscore] = parse_inputs(varargin{:});

%=== defaults
gapopen = -8;
%gapextend = -8; %% DELETED: VESTIGIAL
%setGapExtend = false; %% DELETED: VESTIGIAL
showscore = false;
% isAminoAcid = true; %% DELETED: VESTIGIAL
%isAminoAcid = false; %% CHANGED CODE HERE %% %% DELETED: VESTIGIAL
% scale = 1; %% DELETED: VESTIGIAL
%scoringMatrix = []; %% DELETED: VESTIGIAL
...
if ischar(seq1)
    ...
    intseq1 = fastnt2int(seq1);
else
    intseq1 = uint8(seq1);    
    seq1 = fastint2nt(intseq1); %% CHANGED CODE HERE %%
end
if ischar(seq2)
    seq2 = upper(seq2); %the output alignment will be all uppercase    
    intseq2 = fastnt2int(seq2); 
else
    intseq2 = uint8(seq2);
    seq2 = fastint2nt(intseq2);  %% CHANGED CODE HERE %%
end
...
scoringMatrix = [...
   ...
   -4 -4  1  1 -2 -2 -1 -4 -2 -2 -1 -1 -3 -3 -1;...
    1  1 -4 -4 -2 -2 -4 -1 -2 -2 -3 -3 -1 -1 -1;... %% DELETED CALL TO NUC44
   -4  1  1 -4 -2 -2 -2 -2 -1 -4 -1 -3 -3 -1 -1;... %% AND MOVED CODE HERE
...
%=== call core functions
%% DELETED BECAUSE SECTION IS VESTIGIAL (setGapExtend is always false)
% if setGapExtend  
...
% score = scale * score; %% DELETED: VESTIGIAL
...
if size(path,1) == 0 
    ...
    h = find(all(path>0,2));
%     if isAminoAcid
%         noGaps1 = aa2int(alignment(1,h)); %% DELETED: VESTIGIAL
%         noGaps2 = aa2int(alignment(3,h));
%     else
        noGaps1 = fastnt2int(alignment(1,h));  %% CHANGED CODE HERE %%
        noGaps2 = fastnt2int(alignment(3,h));  %% CHANGED CODE HERE %%
%     end
...
% showscore = false;
% % isAminoAcid = true;
% isAminoAcid = false; %% CHANGED CODE HERE %%
% scale = 1;
% scoringMatrix = [];
%% DELETED SECTION FOR SPEED
